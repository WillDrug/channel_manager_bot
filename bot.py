    ...
    if flavour == 'chat':  # chatting
        ...
        if choice == 'choice':  # TODO: pack those globs into functions
            ...
            if action == 'none':
                ...
                if not ban_check:
                    bot.sendMessage(from_id, f'Okay, chosen!\nSend me something as a submission'
                ...
                if mod is None:
                    ...
                    bot.sendMessage(from_id, help_message(menu='ch_menu'))
                    return True
            elif action == 'admin':
                if ch.owner != from_id:
                    success = False
                    ...
                    context.channel = ch.id
                    session.commit()
                    bot.sendMessage(from_id, help_message(menu='admin'))
                    return True
            else:
        ...
        if command == '/start':  # TODO: fix shitty start
            ...
            # 3) Someone ran /manage on channel and we have to make sure we can send shit to him
                ...
                if context.menu == 'admin_menu':
                    bot.sendMessage(chat_id, help_message(menu='admin'))
                    return True
                else:
                    return True
...
def accept_review(send_to, msg_id):
    bot.sendMessage(send_to, f'Ready to submit!',
                           reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
...
def submit_to_review(msg: Message, session):
    mod = session.query(Mod, func.count(Message.assigned_mod).label('total')).join(Message).group_by(Mod).order_by('total').first()  # TODO: Fix
    if mod is None:
...
def help_message(menu='main'): # TODO: buttons
    if menu == 'admin':
